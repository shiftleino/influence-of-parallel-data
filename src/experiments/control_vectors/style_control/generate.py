...
def main(args):
    ...
    if args.control_vectors_dir:
        control_vector_name = f"control_vector_mean_happy_style_{checkpoint_basename}.pt"
    ...
    print(f"\n{'='*20} All checkpoints processed. {'='*20}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate text completions for ROCStories using Pijiu model checkpoints.")
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing the control vectors. Expected format: 'control_vector_mean_style_{checkpoint_basename}.pt'."
    )
    parser.add_argument(
        "--control_layers",
        type=int,
