...
from pijiu.data import build_train_dataloader
from pijiu.eval import build_evaluators
from pijiu.exceptions import PijiuConfigurationError, PijiuError
from pijiu.model import Pijiu
from pijiu.optim import build_optimizer, build_scheduler
...
from pijiu.train import Trainer
...
def main(cfg: TrainConfig) -> None:
    if cfg.run_name is None:
        raise PijiuConfigurationError("--run_name is required")
    ...
    if torch.cuda.is_available():
        ...
        raise PijiuError("CUDA is not available. Please use a GPU.")
    ...
    if get_global_rank() == 0:
        ...
        if not cfg.dry_run and (cfg.load_path is None or Path(cfg.load_path).parent != Path(cfg.save_folder)):
            ...
            if save_path.is_file() and not cfg.save_overwrite:
                raise PijiuConfigurationError(f"{save_path} already exists, use --save_overwrite to overwrite")
            else:
    ...
    pijiu_model = Pijiu(cfg.model)
    log.info(f"Total number of parameters: {pijiu_model.num_params():,d}")
    log.info(f"Number of non-embedding parameters: {pijiu_model.num_params(include_embedding=False):,d}")
    ...
    if cfg.compile is not None:
        if cfg.model.block_group_size != 1:
            raise PijiuConfigurationError("Compile is only supported with block_group_size 1.")
        for block in pijiu_model.transformer.blocks:
    ...
    if cfg.distributed_strategy == DistributedStrategy.ddp:
        ...
        if cfg.model.init_device != "cuda":
            raise PijiuConfigurationError("DDP does not work with init_device set to anything other than `cuda`.")
        ...
        dist_model = DDP(pijiu_model.to(device), find_unused_parameters=cfg.ddp.find_unused_params)
        ...
        wrap_policy = pijiu_model.get_fsdp_wrap_policy(cfg.fsdp.wrapping_strategy)
        ...
        if cfg.fsdp.sharding_strategy in (ShardingStrategy.HYBRID_SHARD, ShardingStrategy._HYBRID_SHARD_ZERO2):
            ...
            if num_model_replicas <= 0:
                raise PijiuConfigurationError("fsdp.hybrid_sharding_num_model_replicas must be a positive integer")

            if get_world_size() % num_model_replicas != 0:
                raise PijiuConfigurationError("fsdp.hybrid_sharding_num_model_replicas must divide world size")
        ...
        param_init_fn = None
        pijiu_model = pijiu_model.to(device)
        dist_model = SingleAccelerator(pijiu_model)

    # when param_init_fn is None, FSDP will call reset_parameters() automatically
    if param_init_fn is not None or cfg.distributed_strategy == DistributedStrategy.ddp:
        pijiu_model.reset_parameters()
    ...
    if cfg.save_data_indices:
        ...
        if indices_file_path.exists() and not cfg.save_overwrite:
            raise PijiuConfigurationError(f"{indices_file_path} already exists, use --save_overwrite to overwrite")
